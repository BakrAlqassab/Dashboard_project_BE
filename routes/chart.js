...
router.post('/charts', auth, async (req, res) => {
  try {
    const { type, color, sensors } = req.body; 
...
router.delete('/charts/:id', auth, async (req, res) => {
  try {
    const chartId = req.params.id;
    const userId = req.query.userId // Use query parameter if provided, otherwise default to logged-in user's ID


    // Find the chart and check if the user is the owner or an admin
     const chart = await Chart.findOneAndDelete({ _id: chartId, user: userId }); 

    if (!chart) {
      return res.status(404).json({ message: 'Chart not found or you do not have permission to delete it' });
    }
